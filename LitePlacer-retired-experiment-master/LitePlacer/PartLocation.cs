...
namespace LitePlacer {
    [Serializable]
    public class PartLocation : INotifyPropertyChanged {
        private double _a=0, _x=0, _y=0;

        public double X { get { return _x; } set { _x = value; Notify("X"); } }
        public double Y { get { return _y; } set { _y = value; Notify("Y"); } }
        virtual public double A { get { return (_a % 360); } set { _a = (value % 360); Notify("A"); } }
        ...
        public PartLocation(double? x, double? y) { if (x != null) X = (double)x; if (y != null) Y = (double)y; A = 0; }
        public PartLocation(double? x, double? y, double? a) { if (x != null) X = (double)x; if (y != null) Y = (double)y; if (a != null)A = (double)a; }
        ...
        public PartLocation SetTo(PartLocation p) { X = p.X; Y = p.Y; A = p.A; return this; }
        ...
        public Matrix<double> ToMatrix3() { var m = new Matrix<double>(3, 1); m[0, 0] = X; m[1, 0] = Y; return m; }

        public double DistanceTo(PartLocation p) { return Math.Sqrt(Math.Pow(X - p.X, 2) + Math.Pow(Y - p.Y, 2)); }

        public override string ToString() {
            return String.Format("({0:F2},{1:F2},{2:F2})", X, Y, A);
        }
        ...
        public PartLocation RotateDegrees(double degrees)  {
            return Rotate(degrees * Math.PI / 180d);
        }
