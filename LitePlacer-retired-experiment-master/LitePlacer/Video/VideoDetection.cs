...
namespace LitePlacer {
    public class VideoDetection {
        ...
        private static readonly Dictionary<string, Image<Bgr, Byte>> templateCache = new Dictionary<string, Image<Bgr, byte>>();
        private static readonly Dictionary<string, Image<Bgr, Byte>> rotatedTemplateCache = new Dictionary<string, Image<Bgr, byte>>(); 
        ...
        public static List<Shapes.Fiducal> FindTemplates(VideoProcessing vp, Bitmap image, string template_filename, double threshold) {
            if (!File.Exists(template_filename)) return new List<Shapes.Fiducal>();
